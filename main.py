...
st.markdown("""
    ...
    h1 { color: #1e3a8a; font-family: 'Nanum Gothic', sans-serif; }
    h3 { color: #334155; }
    </style>
...
def get_map_resources():
    ...
    for feature in geo_data['features']:
        ...
        if feature['geometry']['type'] == 'MultiPolygon':
            coords = max(feature['geometry']['coordinates'], key=lambda x: len(x[0]))[0]
...
st.caption("2014년 - 2024년 시계열 통합 데이터 기반 상대적 위험도 분석")

uploaded_files = st.sidebar.file_uploader("📂 연도별 CSV 파일 업로드 (다중 선택 가능)", type="csv", accept_multiple_files=True)
full_df = load_and_merge_data(uploaded_files)

if full_df.empty:
    st.info("👈 왼쪽 사이드바에서 분석할 연도별 CSV 파일들을 선택해 주세요.")
    st.stop()
...
m1.metric("선택한 연도", f"{selected_year}년")
m2.metric(f"{option} 평균 중단율", f"{mean_val:.2f}%")
m3.metric("구별 편차(표준편차)", f"{std_val:.2f}")
st.write("---")

# --- 메인 분석 대시보드 ---
tab1, tab2 = st.tabs(["📈 시계열 추이 확인", "🗺️ 자치구별 상대적 위치"])

with tab1:
    st.subheader(f"연도별 {option} 중단율 흐름")
...
with tab2:
    st.subheader(f"{selected_year}년 서울시 지역별 위험도 지도")
