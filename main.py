...
if df is not None:
    ...
    fig_line.update_traces(textposition="top center", textfont=dict(size=14, color="black", family="Arial Black"), line_color="#0083B0", marker=dict(size=10))
    fig_line.update_layout(yaxis=dict(range=[trend_df['학업중단율'].min() * 0.8, trend_df['학업중단율'].max() * 1.3]), margin=dict(t=50))
    fig_line.add_hline(y=avg_val, line_dash="dash", line_color="orange", annotation_text=f"장기 평균({avg_val:.2f}%)")
    ...
    with c_map:
        ...
        st.info("**🔍 지도 해석 가이드**\n* **색상:** 중단 비중(0~2.5%) / **붉은 원:** 실제 중단 학생 규모\n* **진단:** 평균의 1.5배 초과 시 **🔴위기**, 평균 초과 시 **🟡주의**")
    ...
    with c_info:
        ...
        st.divider()
        st.download_button(label="📥 데이터 다운로드 (CSV)", data=map_df.to_csv(index=False).encode('utf-8-sig'), file_name=f"{sel_year}_서울시_학업중단분석.csv", mime='text/csv')
    ...
    with st.expander("💡 히트맵 상세 해석 방법"):
        st.markdown("* **가로 분석**: 오른쪽으로 갈수록 색이 연해지면 예방 성공.\n* **세로 분석**: 특정 연도가 다 진하면 사회적 이슈 발생.\n* **의미**: 짙은 구역은 집중 상담이 필요했던 위기 구간.")
else:
